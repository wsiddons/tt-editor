...
function CropFullHeight({
  ...
  const { botCrop, setBotCrop, setBotPos } = UseCtx();

  useEffect(() => {
    setPosition({
      x: video.current.offsetLeft + 100,
      y: video.current.offsetTop + 100,
    });
    setMaxHeight(video.current.getBoundingClientRect().height);
    setDimensions({ width: cropWidth, height: cropWidth * aspectRatio });
    setBotCrop({ width: cropWidth, height: cropWidth * aspectRatio });
    setBotPos({
      x: video.current.offsetLeft + 100,
      y: video.current.offsetTop + 100,
    });
  ...
  const handleResizingMouseDown = (e) => {
    ...
    setInitialDim({
      width: crop1.current.offsetWidth,
      height: crop1.current.offsetHeight,
    });
  };
  ...
  const handleMouseMove = (e) => {
    if (mouseDown) {
      const boundingX = Math.abs(
        Math.floor(
          crop1.current.getBoundingClientRect().left -
            video.current.getBoundingClientRect().left
        )
      );
      const boundingY = Math.abs(
        Math.floor(
          crop1.current.getBoundingClientRect().top -
            video.current.getBoundingClientRect().top
        )
      );
      setBotPos({
        x: boundingX,
      ...
      if (
        e.clientX - crop1.current.offsetWidth / 2 <
        video.current.offsetLeft
      ) {
        setPosition({
      ...
      //if crop hits top bounds
        ...
        e.clientY - crop1.current.offsetHeight / 2 <
        video.current.offsetTop
      ) {
        setPosition({
